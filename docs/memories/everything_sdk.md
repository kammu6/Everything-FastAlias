...
### Everything 정규식 검색(regex:) API 왜곡 우회 및 사전 조립 기법 (2026-06-29)
- **이슈**: Everything SDK에서 `Everything_SetRegex(true)` API를 명시적으로 세팅하여 검색을 시도할 경우, 쿼리 문자열 전체가 하나의 정규식으로 처리됩니다. 이로 인해 쿼리를 체계적으로 구성하기 위해 덧붙인 괄호 `< >`, 파이프 `|`, 드라이브 기호 `P:`, 제외 수식어 `!$Recycle.Bin`, 크기 제한 `size:` 등 모든 Everything의 고유 예약 연산자 및 예약어가 정규식의 메타문자로 오인되어 문법 오류를 유발하고 결과가 항상 0건이 되는 치명적인 버그가 유발됩니다.
- **해결**:
  1. **API와 파서의 책임 분리**: SDK API 레벨의 `Everything_SetRegex` 호출은 상시 `false`로 격리하여 쿼리 자체의 문법 무결성을 보장합니다.
  2. **개별 토큰 `regex:` 래핑**: 정규식 검색 기능이 활성화되었을 때, 쿼리 변환기 내부에서 일반 검색어 토큰에만 개별적으로 `regex:` 수식어를 동적 결합하여 주입합니다. (예: `BEST` ➔ `regex:BEST`)
  3. **사후 처리(`ApplyModifierToEachTerm`) 전면 제거 및 사전 조립**: 쿼리 조립이 완료된 후 사후에 쿼리를 정규식 기반으로 훑어 `folder:` 수식어를 씌우는 정합성 파괴 방식을 전면 중단했습니다. 대신, 토큰 파싱 시점에 즉각 예약어(`IsConstraintOrDrive`)를 걸러내고 일반 검색어일 때만 `Scope` 및 `modifier`를 pre-compilation 방식으로 적용하여 `folder:path:P:\...` 중복 씌우기 문제를 해결했습니다.
  4. **폴더 검색 시 크기 필터 생략**: 미디어 필터 조건에 "폴더"가 포함된 경우 파일 크기 필터(`size:`)가 아예 생성되지 않도록 제한하여 검색 누락을 방지했습니다.
