...
- **P/Invoke FFI 및 GC 폭풍으로 인한 굉음**: Everything SDK에 `SetMax(0xFFFFFFFF)`를 적용해 무제한 쿼리를 날릴 경우, 빈 쿼리나 광범위한 옵션 변경 시 PC 전체의 수십만 건 파일이 한 번에 반환됨. 이 수십만 개의 데이터를 C# 객체(`SearchResultItem`)로 인스턴스화하고 마샬링(String 복사)하면서 FFI 오버헤드가 발생하고, 가비지 컬렉터(GC)에 심각한 메모리 정리 렉이 걸려 CPU가 폭증하고 팬 굉음이 발생함.
 ...
 ### 18. WPF ListViewItem 드래그 앤 드롭 마우스 간섭 방지 및 더블클릭 우회 지식
 - **현상**: WPF ListView에서 마우스 드래그 오동작 방지 및 다중 선택 상태 유지를 위해 `PreviewMouseLeftButtonDown` 이벤트 핸들러에서 `e.Handled = true`를 선언하면, 윈도우 OS/WPF 시스템의 기본 더블클릭 메시지 생성 흐름이 차단되어 `MouseDoubleClick` 이벤트 자체가 발동하지 않는 문제가 발생함.
 - **해결 방안**: `PreviewMouseLeftButtonDown` 이벤트 처리부 상단에 `e.ClickCount == 2`인 경우(즉, 더블클릭이 감지되었을 때)를 최우선으로 검사하여, 즉시 선택된 아이템의 파일/폴더 경로를 추출하고 `UseShellExecute = true` 옵션을 포함하는 `ProcessStartInfo`를 사용해 OS의 기본 연결 프로그램(이미지 뷰어 등) 또는 윈도우 탐색기로 즉각 기동하고 `e.Handled = true`로 가로챈 뒤 얼리 리턴(Early Return)시킵니다. 이를 통해 드래그 앤 드롭 동작을 방해하지 않으면서도 안정적인 파일 더블클릭 실행 인터랙션을 지원하도록 구현했습니다.
