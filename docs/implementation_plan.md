...
7. **마우스 드래그 다중 선택(Rubber Band Selection) 지원**:
   - 빈 공간 또는 '이름' 컬럼 이외의 셀 영역에서 마우스 드래그 시작 시 반투명 파란색 선택 사각형을 노출.
   - 드래그 영역과 실시간으로 교차(IntersectsWith)하는 리스트뷰 아이템들을 찾아내어 다중 선택 처리.
   - 이름 컬럼 또는 썸네일 카드 영역 내에서 드래그할 때는 기존 파일 드래그앤드롭(외부 앱 전송)이 정상 기동되도록 영역을 격리함.
   - 스크롤바 조작 영역이나 열 헤더 영역 드래그 시에는 발화되지 않도록 철저히 예외 처리.
8. **선택된 항목의 파란색 통일 및 Inactive 회색 렌더링 방지**:
   - 드래그 선택 시작 즉시 리스트 뷰 본체로 포커스를 강제 획득시켜 회색 표시를 1차 예방.
   - 다른 컨트롤로 포커스가 이동하여 리스트 뷰가 비활성 상태가 되더라도 선택 브러시가 회색으로 변하지 않고 파란색 테마 색상으로 유지되도록 `InactiveSelectionHighlightBrushKey`를 오버라이드.
